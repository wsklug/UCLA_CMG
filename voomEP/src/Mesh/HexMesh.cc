...
namespace voom{
  ...
  */
  void HexMesh::Compute() {
    const int numElems = _connectivity.size();
    const int nodePerElem = 8, dimension = 3;
    HexQuadrature quad(_quadOrder); // Quad Object
    Real nds[nodePerElem][dimension]; // Nodal coordinates
