...
namespace voom{
  ...
  */
  void Tri3Mesh::Compute() {
    const int numElems = _connectivity.size();
    const int dimension = 2, nodePerElem = 3;
    Real nds[nodePerElem][dimension]; // Nodal position
    TriangleQuadrature quad(_quadOrder); // Quad Object
	...
	for( int k = 0; k < dimension; k++) 
      ...
      for(Quadrature<2>::ConstPointIterator p = quad.begin(); p!= quad.end();
	  p++){
	ShapeTri3 shp(p->coords);
	const Shape<2>::FunctionContainer &  N = shp.functions();
	const Shape<2>::DerivativeContainer & DN = shp.derivatives();
